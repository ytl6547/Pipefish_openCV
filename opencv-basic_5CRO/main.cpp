...
int main(int argc, char* argv[])
{
	...
	//open CamCapture.avi
	//problem here is no CamCapture.avi file created
	writer.open("CamCapture.avi", 
	-1, default_cam_fps, cvSize(default_cam_width, default_cam_height ), true);
    ...
    while (kb_input != 'q') {
        if (ocam.get_image(image)) {
            ...
            writer.write(image);// save image to CamCapture.avi
